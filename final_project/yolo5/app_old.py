...
def send_request_to_polybot(prediction_id, prediction_summary):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Request failed to {polybot_url}: {response.status_code}")
            return False
    except requests.RequestException as e:
        logger.info(f"Request failed to {polybot_url}: {str(e)}")
        return False
    ...
    while True:
        ...
        if 'Messages' in response:
            ...
            if pred_summary_path.exists():
                ...
                logger.info("path {pred_summary_path} does not exist")
