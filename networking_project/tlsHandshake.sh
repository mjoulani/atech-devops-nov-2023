...
#Client Hello
CLIENT_HELLO_RESPONSE=$(curl -s -X POST "http://43.201.146.120:8080/clienthello" \
...
#Client-Server Master-Key Exchange
...
RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d "$KEY_EXCHANGE" http://43.201.146.120:8080/keyexchange)
#check if the key exchange pass
if [ -z "$RESPONSE" ]; then
    echo "Error: Failed to perform key exchange."
    exit 1
fi
#Parse the encrypted massage
