...
log "Master key generated successfully: $master_key"

# Encrypt master key with a password
encrypted_master_key=$(echo "$master_key" | openssl smime -encrypt -aes-256-cbc -outform DER server_cert.pem | base64 -w 0)
...
decrypted_message=$(openssl enc --aes-256-cbc -d -in encSampleMsgReady.txt -kfile <(echo "$master_key") -pbkdf2)
...
if [ "$decrypted_message" != "$expected_message" ]; then
    log "Decryption does not match the expected message."
    exit 6
fi
