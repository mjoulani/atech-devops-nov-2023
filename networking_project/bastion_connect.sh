...
if [ "$#" -eq 0 ]; then
    ...
    ssh -t -i $KEY_PATH ubuntu@$public_ins "ssh -i Qasem-key-pair.pem ubuntu@$private_ins"
else
    ...
    ssh -t -i $KEY_PATH ubuntu@$public_ins "ssh -i Qasem-key-pair.pem ubuntu@$private_ins '$command'"
fi
